...
const TaskDetailModal = ({ isOpen, onClose, task, isAdmin = true }) => {
  ...
  const isOverdue = task && new Date(task.dueDate) < new Date() && task.status === 'pending'
  const daysUntilDue = task && Math.ceil((new Date(task.dueDate) - new Date()) / (1000 * 60 * 60 * 24))
  ...
  const handleVerify = async (status) => {
    try {
      ...
      toast.success(`Task ${status === 'completed' ? 'verified' : 'rejected'} successfully`)
      onClose()
    } catch (error) {
        ...
        <div className="fixed inset-0 z-[999] overflow-y-auto overflow-x-hidden">
          <div className="flex min-h-full items-center justify-center p-4 sm:p-6 md:p-10 relative">
            ...
            <motion.div 
              ...
              className="relative bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl rounded-[3rem] shadow-2xl border border-white/20 dark:border-slate-800/20 max-w-2xl w-full my-auto overflow-hidden z-20"
            >
              {/* Top Accent */}
              ...
              <div className="max-h-[70vh] overflow-y-auto custom-scrollbar">
                <div className="p-10 space-y-10">
                  ...
                  {isAdmin && task.status === 'submitted' && (
                    <div className="p-10 bg-slate-50/50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 rounded-[2rem]">
                      {!showRejectInput ? (
