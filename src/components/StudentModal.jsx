...
const StudentModal = ({ isOpen, onClose, student }) => {
  ...
  const onSubmit = async (data) => {
    try {
      if (student) {
        await updateStudent.mutateAsync({ id: student._id, ...data })
      } else {
        ...
        <div className="fixed inset-0 z-[999] overflow-y-auto overflow-x-hidden">
          <div className="flex min-h-full items-center justify-center p-4 sm:p-6 relative">
            ...
            <motion.div 
              ...
              className="relative bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl rounded-[3rem] shadow-2xl border border-white/20 dark:border-slate-800/20 max-w-xl w-full my-auto overflow-hidden z-10"
            >
              {/* Background Glow */}
