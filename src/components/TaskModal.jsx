...
import { X, Users, Search, FileUp, FileText, Check, Loader2 } from 'lucide-react'
...
const TaskModal = ({ isOpen, onClose, task }) => {
    ...
    if (isOpen) {
      if (task) {
        reset({
          ...
          assignToAll: task.isGlobal || false
        })
        if (!task.isGlobal && task.tasks) {
          setSelectedStudents(task.tasks.map(t => typeof t.studentId === 'object' ? t.studentId._id : t.studentId))
        }
      } else {
        reset({
          title: '',
  ...
  const onSubmit = async (data) => {
    if (!data.assignToAll && selectedStudents.length === 0) {
      toast.error('Please select at least one personnel unit')
      return
    }
    ...
    try {
      if (isEditMode) {
        if (data.assignToAll) {
          // Logic for edit global
          toast.error('Global edit logic not implemented in backend yet')
        } else {
          await updateTask.mutateAsync({
            id: task.tasks[0]._id, // Using first task ID for group update
            data: {
              ...data,
        ...
        if (data.assignToAll) {
          await bulkCreateTask.mutateAsync({
            ...data,
            isGlobal: true
          })
        } else {
  ...
  const toggleAllInClass = (className) => {
    const classIds = groupedStudents[className].map(s => s._id)
    const allSelected = classIds.every(id => selectedStudents.includes(id))
    
    if (allSelected) {
      setSelectedStudents(prev => prev.filter(id => !classIds.includes(id)))
    } else {
      setSelectedStudents(prev => [...new Set([...prev, ...classIds])])
    }
  }
        ...
        <div className="fixed inset-0 z-[999] overflow-y-auto overflow-x-hidden">
          <div className="flex min-h-full items-center justify-center p-4 sm:p-6 md:p-10 relative">
            ...
            <motion.div 
              ...
              className="relative bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl rounded-[3rem] shadow-2xl border border-white/20 dark:border-slate-800/20 max-w-2xl w-full my-auto overflow-hidden z-10"
            >
              <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-indigo-500 via-purple-500 to-indigo-500" />
              
              <div className="sticky top-0 z-20 px-10 py-8 border-b border-slate-100 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md flex justify-between items-center">
                <div>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="p-10 space-y-10">
                  ...
                  <div className="flex justify-end space-x-6 pt-10 border-t border-slate-100 dark:border-slate-800">
                    ...
                    <motion.button
                      ...
                      className="px-12 py-5 bg-indigo-600 text-white text-[10px] font-black uppercase tracking-[0.2em] rounded-2xl shadow-2xl shadow-indigo-100 dark:shadow-none transition-all disabled:opacity-50"
                    >
                      {isSubmitting ? (
